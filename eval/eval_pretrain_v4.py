"""
Evaluation Script for PDEModelV3 (V4 weights) on Pretrain Datasets.

Evaluates each dataset independently with PDEBench metrics.
Supports 1D, 2D, and 3D datasets (V4 auto-detects from input shape).
...
Usage:
    # Single GPU
    python eval/eval_pretrain_v4.py --config configs/pretrain_v4_s_3D.yaml --checkpoint checkpoints_v4_s_3d_with_best1-2d/best_tf.pt

    # Multi-GPU
    torchrun --nproc_per_node=8 eval/eval_pretrain_v4.py --config configs/pretrain_v4_s_3D.yaml --checkpoint checkpoints_v4_s_3d_with_best1-2d/best_tf.pt

    # Specific datasets only
    torchrun --nproc_per_node=8 eval/eval_pretrain_v4.py --config configs/pretrain_v4_s_3D.yaml --checkpoint checkpoints_v4_s_3d_with_best1-2d/best_tf.pt --datasets 3d_cfd

    # Custom batch size (reduce for 3D to fit in memory)
    torchrun --nproc_per_node=8 eval/eval_pretrain_v4.py --config configs/pretrain_v4_s_3D.yaml --checkpoint checkpoints_v4_s_3d_with_best1-2d/best_tf.pt --batch_size 2
"""
...
class EvalDatasetConfig:
    ...
    Ly: float = 1.0
    Lz: float = 1.0
    vector_dim: int = 0  # 0=no vector, 1=1D velocity, 2=2D velocity, 3=3D velocity


class SingleEvalDataset(Dataset):
    """Dataset for evaluating a single pretrained dataset. Supports 1D, 2D, and 3D."""
    ...
    def _load_metadata(self):
        ...
        with h5py.File(self.path, 'r') as f:
            ...
            if self.has_vector:
                vec_shape = f['vector'].shape  # [N, T, X, 3] or [N, T, H, W, 3] or [N, T, D, H, W, 3]
                self.n_samples, self.n_timesteps = vec_shape[:2]
                # Detect spatial ndim: total dims - 3 (N, T, C)
                self.spatial_ndim = len(vec_shape) - 3  # 1 for 1D, 2 for 2D, 3 for 3D
                self.spatial_shape = vec_shape[2:-1]  # (X,) or (H, W) or (D, H, W)
            elif 'scalar' in f:
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        spatial_shape = vector.shape[:-1]  # (T, X) for 1D, (T, H, W) for 2D, (T, D, H, W) for 3D
...
def load_model(checkpoint_path: str, config: dict, device: torch.device) -> PDEModelV3:
    ...
    # Handle DDP/FSDP/compile prefixes
    new_state_dict = {}
...
def evaluate_dataset(
    ...
    Works for 1D, 2D, and 3D datasets — metric_func auto-detects from tensor shape.
    """
    if accelerator.is_main_process:
        ...
        print(f"Domain: Lx={dataset_config.Lx}, Ly={dataset_config.Ly}, Lz={dataset_config.Lz}")
    ...
    for batch in iterator:
        ...
        # Input/target split
        # 1D data: [B, T, X, 18], 2D data: [B, T, H, W, 18], 3D data: [B, T, D, H, W, 18]
        input_data = data[:, :t_input]
        target_data = data[:, 1:t_input + 1]

        # V4 model auto-detects 1D vs 2D vs 3D from ndim
    ...
    # Pad to equal size across ranks (last batch may be smaller → different counts)
    local_count = torch.tensor([all_preds.shape[0]], device=accelerator.device)
    all_counts = accelerator.gather(local_count)  # [num_ranks]
    max_count = all_counts.max().item()

    if all_preds.shape[0] < max_count:
        pad_n = max_count - all_preds.shape[0]
        all_preds = torch.cat([all_preds, all_preds[:pad_n]], dim=0)
        all_targets = torch.cat([all_targets, all_targets[:pad_n]], dim=0)
    ...
    # Trim to actual total (remove padding duplicates)
    total_actual = all_counts.sum().item()
    all_preds_gathered = all_preds_gathered[:total_actual].cpu()
    all_targets_gathered = all_targets_gathered[:total_actual].cpu()
    ...
    if accelerator.is_main_process:
        ...
        # 3D: [N, D, H, W, C] → [N, D, H, W, 1, C] → metric_func 3D branch
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate PDEModelV4 (V3 arch + 3D)")
    parser.add_argument('--config', type=str, required=True, help='Path to config file')
    parser.add_argument('--checkpoint', type=str, required=True, help='Path to checkpoint')
    parser.add_argument('--batch_size', type=int, default=4, help='Batch size (reduce for 3D)')
    parser.add_argument('--batch_size_3d', type=int, default=1, help='Batch size for 3D datasets')
...
def main():
    ...
    if accelerator.is_main_process:
        print(f"\n{'='*70}")
        print("PDEModelV4 Evaluation (per-dataset, 1D/2D/3D)")
        print(f"{'='*70}")
    ...
    # Dataset configs: 1D + 2D + 3D (matching V4 pretrain datasets)
    dataset_configs = [
        EvalDatasetConfig(
            name='1d_cfd',
        ...
        EvalDatasetConfig(
            name='3d_cfd',
            path=str('/home/msai/song0304/code/PDE/data/pretrained/3D_CFD_unified.hdf5'),
    ...
    for ds_config in valid_configs:
        # Use smaller batch size for 3D datasets (128^3 is memory-heavy)
        bs = args.batch_size_3d if ds_config.vector_dim == 3 else args.batch_size
