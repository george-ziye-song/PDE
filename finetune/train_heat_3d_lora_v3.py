"""
LoRA Finetuning for 3D Heat Equation using PDEModelV3 (V4 weights with 3D support).

Loss: BC loss + PDE loss (NO RMSE - that would be cheating!)
- BC loss: RMSE on 6 boundary faces (supervised anchor for periodic domain)
- PDE loss: Heat equation residual du/dt = alpha * laplacian(u)

Channel mapping (18-channel):
- Channel 17 = scalar[14] = temperature
- No velocity fields (vector_dim=0)

Special: per-sample alpha (each trajectory has different thermal diffusivity)

Uses V4 pretrained weights (V3 architecture with enable_3d=true).
LoRA on: NA Attention (qkv, proj) + FFN (gate_proj, up_proj, down_proj)
...
if not IS_MAIN_PROCESS:
    ...
    # Suppress flex_attention unfused warning (4D NA uses flex backend, torch.compile
    # backward is unstable so we must run unfused — this warning is expected)
...
CH_TEMP = 17  # scalar[14] = temperature (3 + 14 = 17)


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for 3D Heat Equation (V4 Model)")
...
def compute_boundary_loss(
    ...
    """
    Compute boundary RMSE on 6 faces (3D periodic domain).

    pred/target: [B, T, X, Y, Z, 18]
    """
    ...
    faces = []
    for tensor in (pred, target):
        # x faces
        faces.append(tensor[:, :, 0, :, :, :][:, :, :, :, valid_ch])
        faces.append(tensor[:, :, -1, :, :, :][:, :, :, :, valid_ch])
        # y faces
        faces.append(tensor[:, :, :, 0, :, :][:, :, :, :, valid_ch])
        faces.append(tensor[:, :, :, -1, :, :][:, :, :, :, valid_ch])
        # z faces
    ...
    # First 6 = pred faces, last 6 = target faces
    bc_pred = torch.cat([f.reshape(-1) for f in faces[:6]])
    bc_target = torch.cat([f.reshape(-1) for f in faces[6:]])
...
def compute_pde_loss(
    ...
    """
    Compute Heat equation PDE residual loss with per-sample alpha.

    Args:
        output: Model output [B, T_out, X, Y, Z, 18] (denormalized)
        input_data: Model input [B, T_in, X, Y, Z, 18]
        pde_loss_fn: Heat3DPDELoss instance
        alpha: Per-sample thermal diffusivity [B]
    ...
    with torch.autocast(device_type='cuda', enabled=False):
        ...
        t0_u = input_data[:, 0:1, :, :, :, CH_TEMP].float()
        out_u = output[:, :, :, :, :, CH_TEMP].float()

        # [B, T_out+1, X, Y, Z] for 2nd-order central time difference
...
def validate(
    ...
    for batch in val_loader:
        ...
        alpha = batch['nu'].to(device=accelerator.device)  # 'nu' key holds alpha
        ...
        # PDE loss (denormalized, per-sample alpha)
...
def main():
    ...
    # Create PDE loss function (alpha is per-sample, passed at forward time)
    ...
    pde_loss_fn = Heat3DPDELoss(
        ...
        Lz=physics.get('Lz', 6.283185307179586),
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"heat_3d-lora-v4-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["heat_3d", "lora", "v4", "3D", "BC+PDE", "per-sample-alpha"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # BC loss (6 faces as supervised anchor)
                    bc_loss = compute_boundary_loss(output, target_data, channel_mask)

                    # PDE loss (heat equation, per-sample alpha)
