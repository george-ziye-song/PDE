"""
LoRA Finetuning for POSEIDON NS-PwC (Incompressible NS + Tracer, periodic BC).
...
- Channel 3 = scalar[0] = tracer (scalar_indices=[0])
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_ns_pwc_lora_v3.py --config configs/finetune_ns_pwc_v3.yaml
"""
...
CH_VX = 0       # vector: Vx
CH_VY = 1       # vector: Vy
CH_TRACER = 3   # scalar[0] = tracer (scalar_indices=[0])


def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for NS-PwC (V3 Model)")
...
def compute_pde_loss(
    ...
    """
    Compute NS-PwC PDE residual loss (divergence + vorticity + tracer).
    ...
    Args:
        ...
        pde_loss_fn: NSPwCPDELoss instance
...
def main():
    ...
    if accelerator.is_main_process:
        logger.info(f"{'='*60}")
        logger.info("NS-PwC LoRA Finetuning (V3 Model)")
        logger.info(f"{'='*60}")
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"ns_pwc-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["ns_pwc", "lora", "v3", "periodic", "BC+PDE", "normalized"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # PDE loss (div + vorticity + tracer, per-equation normalized)
