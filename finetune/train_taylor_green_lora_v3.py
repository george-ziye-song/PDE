"""
LoRA Finetuning for Taylor-Green Vortex 2D (Incompressible NS) using PDEModelV3.
...
- PDE loss: Continuity + Momentum (u,v) with per-sample nu

Channel mapping (18-channel):
- Channel 0 = Vx, Channel 1 = Vy (vector_dim=2)
- Channel 15 = scalar[12] = pressure
- No tracer (unlike Shear Flow)

Special: per-sample viscosity nu (each trajectory has different Re)
...
Usage:
    torchrun --nproc_per_node=8 finetune/train_taylor_green_lora_v3.py --config configs/finetune_taylor_green_v3.yaml
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA Finetuning for Taylor-Green 2D (V3 Model)")
...
def compute_pde_loss(
    ...
    Extracts u (ch0), v (ch1), pressure (ch15) from denormalized
    model output, prepends t0 frame for time derivatives.

    Args:
        output: Model output [B, T_out, H, W, 18] (denormalized)
        input_data: Model input [B, T_in, H, W, 18]
        pde_loss_fn: TaylorGreenPDELoss instance
...
def validate(
    ...
    for batch in val_loader:
        ...
        # PDE loss (denormalized, per-sample nu)
...
def main():
    ...
    # Create PDE loss function (nu is per-sample, passed at forward time)
    ...
    pde_loss_fn = TaylorGreenPDELoss(
        ...
        nu=physics.get('nu_default', 0.001),  # fallback only; per-sample nu used
    )
    ...
    if accelerator.is_main_process:
        ...
        run_name = f"taylor_green-lora-v3-r{lora_r}"
        accelerator.init_trackers(
            ...
            init_kwargs={"wandb": {
                ...
                "name": run_name,
                "tags": ["taylor_green", "lora", "v3", "shared-ffn", "BC+PDE", "per-sample-nu"],
            }}
        )
        ...
        for epoch in range(start_epoch, max_epochs):
            ...
            for batch in train_loader:
                ...
                with accelerator.accumulate(model):
                    ...
                    # PDE loss (continuity + momentum, per-sample nu)
