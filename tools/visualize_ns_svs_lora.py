"""
LoRA V3 Model Visualization for NS-SVS (Incompressible NS + Tracer, periodic BC).
...
Usage:
    ...
    torchrun --nproc_per_node=8 tools/visualize_ns_svs_lora.py \
        --config configs/finetune_ns_svs_v3.yaml \
        --checkpoint checkpoints_ns_svs_lora_v3/best_lora.pt --scan_all

    # Single-GPU visualization (plot only)
    python tools/visualize_ns_svs_lora.py \
        --config configs/finetune_ns_svs_v3.yaml \
        --checkpoint checkpoints_ns_svs_lora_v3/best_lora.pt --output_dir ./ns_svs_vis
"""
...
# NS-SVS channel indices in 18-channel layout (same as NS-PwC)
CH_VX = 0       # vector: Vx
CH_VY = 1       # vector: Vy
...
def parse_args():
    parser = argparse.ArgumentParser(description="LoRA V3 Visualization for NS-SVS")
...
def run_visualization(model, dataset, config, device, t_input, num_samples, seed, output_dir):
    ...
    plot_velocity(results, str(output_dir / "visualization_ns_svs_velocity.png"))

    print("Plotting tracer visualization...")
    plot_tracer(results, str(output_dir / "visualization_ns_svs_tracer.png"))

    print(f"\n{'='*60}")
    print("Visualization Complete (NS-SVS LoRA)")
    print(f"{'='*60}")
...
def main():
    ...
    if is_main:
        print(f"{'='*60}")
        print("NS-SVS LoRA V3 Evaluation")
        print(f"{'='*60}")
        ...
        if is_main:
            ...
            if results.get('per_clip'):
                ...
                plot_loss_distribution(
                    results['per_clip'],
                    str(ckpt_dir / "loss_distribution_ns_svs.png"),
                )
