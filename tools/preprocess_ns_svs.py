"""
Preprocess POSEIDON NS-SVS NetCDF data to our HDF5 format.
...
NS-SVS uses the same data format as NS-PwC (same AZEBAN solver, same channels).
Only the initial condition differs (sinusoidal vortex sheet vs piecewise constant).
...
Usage:
    python tools/preprocess_ns_svs.py \
        --input_dir /scratch-share/SONG0304/finetune/NS-SVS \
        --output /scratch-share/SONG0304/finetune/ns_svs.hdf5
"""
...
def main():
    parser = argparse.ArgumentParser(description="Preprocess NS-SVS NetCDF -> HDF5")
